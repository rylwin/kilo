...
void editorUpdateSyntax(erow *row) {
  ...
  while (i < row->rsize) {
    char c = row->render[i];
    unsigned char prev_hl = (i < 0) ? HL_NORMAL : row->hl[i - 1];
...
void editorSelectSyntaxHighlight() {
  ...
  for (unsigned int i = 0; i < HLDB_ENTRIES; i++) {
    ...
    for (unsigned int j = 0; s->filematch[j]; j++) {
      char *match = strstr(E.filename, s->filematch[j]);
      if (match) {
        int patlen = strlen(s->filematch[j]);
        if (s->filematch[j][0] != '.' || match[patlen] == '\0') {
          E.syntax = s;
...
void editorInsertNewline() {
  if (E.cx == 0) {
    ...
    // Get the row again: editorInsertRow uses realloc and may have moved data
    row = currentRow();
    row->size = E.cx;
    row->chars[row->size] = '\0';
    editorUpdateRow(row);
...
void editorSetStatusMessageIoError() {
  ...
  switch (c) {
    ...
    case CTRL_KEY('q'):
      if (E.dirty && quit_times > 0) {
        editorSetStatusMessage("WARINING!!! File has unsaved changes. "
    ...
    case HOME_KEY:
      ...
      break;
    case END_KEY:
      if (E.cy < E.numrows) E.cx = currentRow()->size;
      break;
    case BACKSPACE:
    case CTRL_KEY('h'):
    case DEL_KEY:
